        ...
        node.on("input", function(msg, send, done) {
            try {
                const getVal = (key, staticVal, isProp = false) => {
                    ...
                    if (isProp) {
                        return RED.util.evaluateNodeProperty(staticVal, config[`${key}Type`] || "str", node, msg);
                    }
                    return staticVal;
                };
                ...
                if (enableTables === false || enableTables === "false")
                    args.push("--no-tables");
                else {
                ...
                if (!fs.existsSync(venvBinPath)) {
                    const errMsg = "Local Docling environment not found. Run 'npm run postinstall' in the node directory.";
